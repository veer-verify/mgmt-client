   ...
   constructor(
    ...
    createIssueForFaq() {
      this.addAssetForm.value.createdBy = this.user?.UserId
        ...
        // this.alertSer.wait();
        if(this.addAssetForm.value.zoneId === null) {
          this.addAssetForm.value.zoneId = this.newZoneId;
        }
        this.adver.linkZoneIdToSiteId(this.addAssetForm.value).subscribe((res: any) => {
          // console.log(res);
    ...
    zonesList:any = [];
    listAvailableZones() {
      this.adver.listAvailableZones().subscribe((res:any) => {
        // console.log(res)
        this.zonesList = res.zonesList
      })
    }
    ...
    newZoneId: any;
    createZone() {
      this.adver.createZone(this.addAssetForm.value.addZoneForm).subscribe((res:any) => {
        this.newItemEvent.emit();
        if(res.statusCode == 200 ) {
          this.newZoneId = res.zoneId;
          this.createIssueForFaq();
          this.alertSer.success(res?.message)
        } else {
          this.alertSer.error(res?.message)
        }
